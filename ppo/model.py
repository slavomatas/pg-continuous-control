...
class ActorBody(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_units=(256, 256), gate=F.relu):
...
class GaussianActorCriticNet(nn.Module, BaseNet):
    ...
    def forward(self, obs, action=None):
        ...
        dist = torch.distributions.Normal(mean, torch.cuda.FloatTensor(self.network.actor_body.std))
        if action is None:
            action = dist.sample()
